...
public class SwiftPopMenu: UIView {
    ...
    // 背景色
    private var popMenuBgColor: UIColor = .white
    // 圆角弧度
...
class SwiftPopMenuCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        iconImage.frame = CGRect(x: iconLeft, y: (bounds.size.height - 20) / 2, width: 20, height: 20)
        lblTitle.frame = CGRect(x: 20 + iconLeft * 2, y: 0, width: bounds.size.width - 40, height: bounds.size.height)

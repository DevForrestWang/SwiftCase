...
class ScreenMediator: ScreenUpdatable {
    ...
    func likedAdded(to news: SMNews) {
        SC.log("Screen Mediator: Received a liked news model with id \(news.id)")
        screens?.forEach { $0.likedAdded(to: news) }
    }

    func likeRemoved(from news: SMNews) {
        SC.log("ScreenMediator: Received a disliked news model with id \(news.id)")
...
class NewFeedViewController: ScreenUpdatable {
    ...
    func likedAdded(to news: SMNews) {
        SC.log("News Feed: Received a liked news model with id \(news.id)")
    ...
    func likeRemoved(from news: SMNews) {
        SC.log("News Feed: Received a disliked news model with id \(news.id)")
    ...
    func userLikedAllNews() {
        ...
        SC.log("News Feed: I am telling to mediator about it...\n")
...
class NewDetailViewController: ScreenUpdatable {
    ...
    func likedAdded(to news: SMNews) {
        SC.log("News Detail: Received a liked news model with id \(news.id)")
        if self.news == news {
    ...
    func likeRemoved(from news: SMNews) {
        SC.log("News Detail: Received a disliked news model with id \(news.id)")
        if self.news == news {
            self.news.likeCount -= 1
        }
    }
...
class ProfileViewController: ScreenUpdatable {
    ...
    func likedAdded(to news: SMNews) {
        SC.log("Profile: Received a liked news model with id \(news.id)")
        numberOfGivenLikes += 1
    }

    func likeRemoved(from news: SMNews) {
        SC.log("Profile: Received a disliked news model with id \(news.id)")
        numberOfGivenLikes -= 1
    }
}

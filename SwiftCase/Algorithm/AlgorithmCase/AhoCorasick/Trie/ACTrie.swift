...
public struct ACTrie {
    private var config: TrieConfig
    private let rootState = ACState()
    ...
    public func tokenize(text: String) -> [ACToken] {
        var tokens = [ACToken]()
    ...
    private func createFragment(_ emit: ACEmit?, text: String, lastCollectedPosition: Int) -> ACToken {
    ...
    private func createMatch(_ emit: ACEmit, text: String) -> ACToken {
    ...
    public func parse(text: String) -> [ACEmit] {
    ...
    private func collectEmits(for text: String) -> [ACEmit] {
        var currentState = rootState
        var storedEmits = [ACEmit]()
    ...
    public func firstMatch(text: String) -> ACEmit? {
        if config.removeOverlaps {
            let parseText = parse(text: text)
            ...
            for (position, character) in text.enumerated() {
                ...
                for string in emitStrings {
                    let emit = ACEmit(start: position - string.count + 1, end: position, keyword: string)
                    if config.onlyDelimited {
    ...
    private func isPartialMatch(searchText: String, emit: ACEmit) -> Bool {
    ...
    private func removePartialMatches(searchText: String, collectedEmits: [ACEmit]) -> [ACEmit] {
        let nonPartialMatches = collectedEmits.filter { !isPartialMatch(searchText: searchText, emit: $0) }

        return nonPartialMatches
    }

    private func getState(currentState: ACState, character: Character) -> ACState {
    ...
    private func storeEmits(position: Int, currentState: ACState) -> [ACEmit] {
        let emits = currentState.emits

        var storedEmits = [ACEmit]()

        for emit in emits {
            storedEmits.append(ACEmit(start: position - emit.count + 1, end: position, keyword: emit))
        }
